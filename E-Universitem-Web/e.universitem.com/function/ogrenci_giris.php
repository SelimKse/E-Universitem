...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $ogrenci_email = filter_var($data->email, FILTER_SANITIZE_EMAIL);
    $ogrenci_password = $data->password;
    $beni_hatirla = isset($data->beniHatirla) ? $data->beniHatirla : false;

    // SQL sorgusu - Öğrenci bilgilerini kontrol et
    $sql = "SELECT * FROM ogrenciler WHERE ogrenci_eposta = :email";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':email', $ogrenci_email);
    $stmt->execute();
    ...
    if ($ogrenci) {
        if (password_verify($ogrenci_password, $ogrenci['ogrenci_sifre'])) {
            // Oturum bilgilerini ayarla
            $_SESSION['isim'] = $ogrenci['ogrenci_adi'];
            $_SESSION['email'] = $ogrenci['ogrenci_eposta'];
            $_SESSION['id'] = $ogrenci['ogrenci_no'];
            ...
            $token_stmt->execute([
                ':user_id' => $ogrenci['ogrenci_no'],
                ':user_type' => 'ogrenci',  // Öğrenci olduğunu belirtiyoruz
                ':token' => $token,
            ...
            $response = [
                ...
                'user' => [
                    'isim' => $ogrenci['ogrenci_adi'],
                    'email' => $ogrenci['ogrenci_eposta'],
                    'id' => $ogrenci['ogrenci_no']
                ],
                'token' => $token,
            ];
        } else {
