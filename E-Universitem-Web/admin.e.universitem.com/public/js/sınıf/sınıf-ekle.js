...
async function sınıfEkle(event) {
  event.preventDefault(); // Sayfanın yenilenmesini engelle

  // Formdan verileri al
  const sınıfAdı = document.getElementById("sınıfAdı").value;

  // fetch API ile PHP dosyasına POST isteği gönder
  const response = await fetch("../../../function/admin/sınıf_ekle.php", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ sınıf_adı: sınıfAdı }),
  });
  ...
  if (result.status === "success") {
    ...
    document.getElementById("sınıfAdı").value = "";
  } else {
    Toast.fire({
      icon: "error",
