...
searchInput.addEventListener("keyup", function () {
  ...
  if (!noResultsItem) {
    ...
    ogretmenlerList.appendChild(noResultsItem);
  }
  ...
  if (!noInputItem) {
    ...
    noInputItem.textContent = "Lütfen öğretmen ismi veya numarası giriniz";
    noInputItem.style.display = "none"; // Başlangıçta gizli
    ogretmenlerList.appendChild(noInputItem);
  }
  ...
  if (filter === "") {
    ...
    for (let i = 0; i < ogretmenler.length; i++) {
      ogretmenler[i].style.display = "none";
    }
  ...
  for (let i = 0; i < ogretmenler.length; i++) {
    ...
    const ogretmenText = ogretmenler[i].textContent.toLowerCase();
    if (ogretmenText.includes(filter)) {
      ogretmenler[i].style.display = ""; // Eşleşenleri göster
      found = true; // Eşleşme bulundu
    } else {
      ogretmenler[i].style.display = "none"; // Eşleşmeyenleri gizle
    }
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!noResultsItem) {
    noResultsItem = document.createElement("li");
    noResultsItem.id = "noResults";
    noResultsItem.textContent = "Öğretmen bulunamadı";
    noResultsItem.style.display = "none"; // Başlangıçta gizli
    ogretmenlerList.appendChild(noResultsItem);
  }

  // Tüm li etiketlerini seç
  const listItems = document.querySelectorAll("#ogretmenler > li");
  ...
  listItems.forEach(li => {
    li.addEventListener("click", async function (event) {
      ...
      const id = li.id;
      const ogretmen = await fetch("../../function/admin/ogretmen_bilgi.php", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const result = await ogretmen.json();
      let dersler = result.ogretmen.verdigi_dersler;
...
async function dersKaydet(event, id) {
  ...
  const derskaydet = await fetch("../../function/admin/ogretmen_ders_tanımlama.php", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
