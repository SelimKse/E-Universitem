...
const form = document.getElementById("ogrenciEkleForm");
...
async function ogrenciEkle(event) {
  event.preventDefault(); // Sayfanın yenilenmesini engelle
  const ogrenciNo = Number(document.getElementById("ogrenciNo").value);
  const ogrenciAdı = document.getElementById("ogrenciAdı").value;
  const ogrenciSoyadı = document.getElementById("ogrenciSoyadı").value;
  const ogrenciEmail = document.getElementById("ogrenciEposta").value;
  const ogrenciPhoneNo = document.getElementById("ogrenciPhoneNo").value;
  const ogrenciSifre = document.getElementById("ogrenciSifre").value;

  const ogrenciEkleForm = document.getElementById("ogrenciEkleForm");

  if (ogrenciEkleForm.checkValidity()) {
    const response = await fetch("../../function/admin/ogrenci_ekle.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
