...
window.addEventListener("load", async () => {
  ...
  if (yoklamalar.length === 0) {
    ...
    noResultsItem.textContent = "Yoklama bulunamadı!"; // Mesajı ayarla
  ...
  // bütün yoklamaların ogretmen_adi classlarınin içindeki textContent'ini düzenle
  for (let i = 0; i < yoklamalar.length; i++) {
    const yoklama = yoklamalar[i].getElementsByClassName("ogretmen_adi")[0];
    const ogretmen_no = yoklama.getAttribute("data-id");
    const ogretmenDetayFunc = await ogretmenDetay(ogretmen_no);
    yoklama.textContent = ogretmenDetayFunc;
  }
});
