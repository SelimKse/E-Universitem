...
async function veriler() {
  ...
  if (!sessionResponse.status) {
    window.location.href = "https://e-universitem.com/auth/ogretmen/giris.php";
    return;
  }
  ...
  const request = await fetch("../../function/yoklamalar.php", {
    ...
    body: JSON.stringify({
      ogretmen_no: sessionResponse.data.ogretmen_no,
    }),
  });
  ...
  const bildirimler = await fetch("../../function/bildirimler.php", {
    ...
    body: JSON.stringify({
      user_id: sessionResponse.data.ogretmen_no,
    }),
  });
